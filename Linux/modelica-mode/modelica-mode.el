...
(defconst modelica-mode-version "1.5")
...
;;Helper function for FlatModelica
(defun process-dots-within-single-quotes-excluding-brackets ()
  (goto-char (point-min))
  (while (re-search-forward "'[^']*'" nil t)
    (let* ((match-str (match-string 0)))
      (unless (string-match-p "\\[" match-str)
        (let ((processed-str (replace-regexp-in-string "\\." "_" match-str)))
          (replace-match processed-str nil t))))))

(defun remove-quotes-except-brackets ()
  (goto-char (point-min))
  (while (re-search-forward "'\\([^']*\\)'" nil t)
    (let* ((match-str (match-string 1)))
      (unless (string-match-p "\\[" match-str)
        (replace-match match-str nil t)))))

(defun reformat-flat-modelica ()
  "Removes quoted expressions that does not need to be quoted.
  Note this function assumes that the file does not contain any other characters than ASCII."
  (interactive)
  (progn
    (my-process-dots-within-single-quotes-excluding-brackets)
    (remove-quotes-except-brackets)))
